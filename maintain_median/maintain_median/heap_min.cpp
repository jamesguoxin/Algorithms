...
int heap_min::Min_Value() {
    return heap_array[0];
}

int heap_min::heap_size() {
    return int(heap_array.size());
}

void heap_min::Insert(int key) {
    heap_array.push_back(key);
    Bubble_Up(int(heap_array.size() - 1));
}

void heap_min::Bubble_Down(int position) {
...
void heap_min::Bubble_Up(int position) {
